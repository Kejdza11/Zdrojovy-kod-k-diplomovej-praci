...
% Naèítanie vstupných parametrov :
input_params;  % Naèíta vstupy zo skriptu input_params.m

% Spustenie CYKLU REBALANSOVANIA :
for i = 1:num_iterations
    % Definovanie indexov pre kåzavé okno :
    start_idx = (i-1)*test_size + 1;
    end_idx = start_idx + window_size - 1;
    
    % Výber dát pre optimalizáciu :
    data_window = returns(start_idx:end_idx, :);
    
    % Množina neistoty oèakávaných výnosov z èasových úsekov :
    rolling_mean_vectors = rolling_mean_uncertainty(data_window, test_size);
    
    % Optimalizácia na výpoèet váh :
    cvx_quiet(true); % Stíšenie zobrazovania výstupov CVX
    ...
    % Uloženie váh a výnosov :
    mean_variance_weights = [mean_variance_weights; mv_weights']; % váhy sa mi tu ukladajú ako riadky
    ...
    % Na overenie, èi diagonal portfólio investovalo vždy do akcie s najlepším Sharpe pomerom :
...
% Výber akcie pri Diagonal portfóliu pod¾a max. úèelovej funkcie :
...
% Absolútny vývoj kapitálu - výpoèet kumulatívneho kapitálu:
...
alpha = 0.05; % hranica
returns_last_year = returns(505:756, :); % Vyberieme posledných 252 dní
...
% Tabu¾ky numerických výsledkov metrík :
